...
final class GramToKcalConverterTests: XCTestCase {
    ...
    func test_引数で受け渡すgrams（Double型）の値が計算の過程で小数第二位の値に応じて四捨五入されていることを確認() {
        ...
        let proteinKcalOfRoundedDown: Int = GramToKcalConverter.getProteinKcal(for: proteinGramsOfToRoundDown)
        ...
        let proteinKcalOfRoundedUp: Int = GramToKcalConverter.getProteinKcal(for: proteinGramsOfToRoundUp)
        ...
        let fatKcalOfRoundedDown: Int = GramToKcalConverter.getFatKcal(for: fatGramsOfToRoundDown)
        ...
        let fatKcalOfRoundedUp: Int = GramToKcalConverter.getFatKcal(for: fatGramsOfToRoundUp)
        ...
        let carbohydrateKcalOfRoundedDown: Int = GramToKcalConverter.getCarbohydrateKcal(for: carbohydrateGramsOfToRoundDown)
        // 1.125 x 4 = 4.5 だと繰り上がって、結果が5になるはず。
        // 但し、実際には1.125 は 1.1に四捨五入された状態で計算が行われるので、
        // 1.1   x 4 = 4.4 で切り捨てになって、結果が4になることを確認する。
        ...
        let carbohydrateKcalOfRoundedUp: Int = GramToKcalConverter.getCarbohydrateKcal(for: carbohydrateGramsOfToRoundUp)
        // 1.35 x 4 = 5.4 だと切り捨てになって、結果が5になるはず。
        // 但し、実際には1.35 は 1.4に四捨五入された状態で計算が行われるので、
        // 1.4  x 4 = 5.6 で繰り上がって、結果が6になることを確認する。
    ...
    func test_各PFCごとのgramToKcal変換の基準となる値に対して期待する計算結果になっていることを確認() {
        ...
        let proteinKcalFor3Grams: Int = GramToKcalConverter.getProteinKcal(for: 3)
        // 3 x 4 = 12
        XCTAssertEqual(proteinKcalFor3Grams, 12)
        
        let fatKcalFor3Grams: Int = GramToKcalConverter.getFatKcal(for: 4)
        // 4 x 9 = 36
        XCTAssertEqual(fatKcalFor3Grams, 36)
        
        let carbohydrateKcalFor3Grams: Int = GramToKcalConverter.getCarbohydrateKcal(for: 5)
        // 5 x 4 = 20
